...
class AbstractKerberosAuthHandler(BaseHandler):
    ...
    def negotiate_value(self, headers):
        ...
        if authreqs:
            ...
            log.debug("{self.auth_header} header not found")

        return None

    def generate_request_header(self, req, headers, neg_value):
        self.retried += 1
        log.debug("retry count: %s", self.retried)
        ...
        result, self.context = k.authGSSClientInit(f"HTTP@{domain}", **kwargs)

        if result < 1:
            log.warning("authGSSClientInit returned result %s", result)
            return None
        ...
        if result < 0:
            log.warning("authGSSClientStep returned result %s", result)
            return None
    ...
    def authenticate_server(self, headers):
        ...
        if result < 1:
            ...
            log.critical(
                "mutual auth failed: authGSSClientStep returned result %s", result
            )
...
class ProxyKerberosAuthHandler(AbstractKerberosAuthHandler):
...
class HTTPKerberosAuthHandler(AbstractKerberosAuthHandler):
